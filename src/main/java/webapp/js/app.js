...
/*---------------------------------------------------------------------
  ...
  /*---------------------------------------------------------------------
    ...
    /* ----------------- Chức năng thêm action ------------------ */
  
  let addToStoreBtn = document.querySelector('.add-to-store');
  let chkAllBtn = document.querySelector(".checkAll");
  let productChk = document.querySelectorAll('.chkProduct');
  let action = document.querySelector('.action');
  let unchkAllBtn = document.querySelector('.unchkAllBtn');
  if(unchkAllBtn != null) {
	  unchkAllBtn.addEventListener('click', handleUnchkAllbtnClick);
  }
	if(productChk != null && productChk.length > 0) {
		productChk.forEach(item => item.addEventListener('change', handleCheckProduct))
	}
  if(addToStoreBtn != null && addToStoreBtn.length > 0) {
	  addToStoreBtn.forEach(item => item.addEventListener('change', handleAddToStoreBtnClick))
  }
  /**
   * Khi người dùng check 1 product, nếu số lượng product được check lớn hơn 0 thì sẽ hiện thêm hộp chọn action và số lượng sản phẩm được chọn
   * Bên cạnh số lượng được chon sẽ có nút x để hủy chọn tất cả sản phẩm
   * Trên header của table sẽ có hộp chọn để chọn hoặc hủy chọn tất cả sản phẩm trong trang
   * Khi người dùng click vào action thêm vào kho, nếu số lượng product check > 0 thì sẽ chuyển sang trang nhập sản phâm
   * Trong danh sách kho hàng để chọn, trên mối dòng khi thì sẽ có cột thêm vào kho. nếu người dùng đã ấn vào nút đó thì sẽ thay nội dung là hủy
   */
  function handleCheckProduct() {
	
	var productsChecked = document.querySelectorAll('input[type="checkbox"].chkProduct:checked');
	var productsChk = document.querySelectorAll('input[type="checkbox"].chkProduct');
	document.querySelector('.count').innerHTML = productsChecked.length;
	 if(productsChecked != null && productsChecked.length > 0) {
		 if(action.hidden) {
			 action.removeAttribute('hidden');
		 }
		 if(productsChecked.length == productChk.length) {
			 chkAllBtn.checked = false;
		 }
	 } else {
		 if(!action.hidden) {
			 action.hidden = true;
  ...
  if(chkAllBtn != null) {
	  chkAllBtn.addEventListener('click', handleChkAllbtn);
	  
  }
  function handleChkAllbtn(e) {
	  var productsChecked = document.querySelectorAll('input[type="checkbox"].chkProduct');
	  if(e.target.checked) {
		  productsChecked.forEach(item => item.checked = true);
		  document.querySelector('.count').innerHTML = productsChecked.length;
		  if(action.hidden) {
			 action.removeAttribute('hidden');
		 }
		  ...
		  productsChecked.forEach(item => item.checked = false);
		  if(!action.hidden) {
			 action.hidden = true;
		 } 
	  }
  ...
  function handleAddToStoreBtnClick() {
	  var productsChecked = document.querySelectorAll('input[type="checkbox"].chkProduct:checked');
	if(productsChecked != null && productsChecked.length > 0) {
		
	}
  }
  function handleUnchkAllbtnClick() {
	  chkAllBtn.checked = false;
	  document.querySelectorAll('input[type="checkbox"].chkProduct').forEach(item => item.checked = false);
	  if(!action.hidden) {
		 action.hidden = true;
	  } 
  }
  
  /* ----------------- Chức năng thêm action end ------------------ */
  
  let inputImport = document.getElementById("import-excel");
	inputImport.addEventListener('change', () => console.log(inputImport.files[0]));
})(jQuery);
